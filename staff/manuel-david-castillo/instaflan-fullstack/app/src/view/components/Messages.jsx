...
export default function Messages() {
    ...
    return <section className="flex flex-col items-center pb-20 w-full">
        {chats?.map(chat => <article onClick={(event) => handleNavigateChat(event, chat.id)} key={chat.id} className="w-full flex flex-col border-b-2 border-b-gray-400 p-1 hover:bg-gray-200 cursor-pointer">
            ...
            <p className="m-2 ml-3 mb-0">{chat.messages[chat.messages.length - 1]?.text}</p>
        </article>)}
    </section>
}