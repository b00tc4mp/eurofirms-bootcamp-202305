...
describe('retrievePanelOne', () => {
    ...
    it('succeeds on retrievePanelOne', () =>
        retrievePanelOne(userId, panelId)
            .then(panel => expect(panel.id).to.be.equal(panelId))
    )

    it('fails on user does not exist', () =>
        retrievePanelOne(userId.slice(6) + 'ffffff', panelId)
            .catch(error => {
