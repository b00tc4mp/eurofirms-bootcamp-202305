 ...
 * @returns a promise.
 */
function registerUserDB(name, surname, zip, email, password) {
    ...
    return User.findOne({ email }, '_id').lean()
        .then((user) => {
            ...
            return User.create({ name, surname, zip, email, password })
        })
        .then(() => { })
}
/**
 ...
 */
function authenticateUserDB(email, password) {
    validateString(email, validateString.EMAIL)
    validateString(password, validateString.PASSWORD)
 ...
 */
function retrieveUserDB(id) {
    validateString(id)

    return User.findById(id, 'name surname zip email').lean()
        .then((user) => {
            if (!user) throw new Error('El usuario no existe')
