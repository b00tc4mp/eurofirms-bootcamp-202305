...
describe('deletePanel', () => {
    ...
    it('succeeds on deletePanel', () =>
        deletePanel(userId, panelId)
            .then(() => PanelModel.findById(panelId)
                .then(panel => expect(panel).to.be.equal(null))
            ))

    it('fails on user does not exist', () =>
        deletePanel(userId.slice(6) + 'ffffff', panelId)
            .catch(error => {
    ...
    it('fails on panel does not exist', () =>
        deletePanel(userId, panelId.slice(6) + 'ffffff')
            .catch(error => {
                expect(error).to.be.instanceOf(Error)
