...
const writeWorkingPanel = function (panel) {
    // fs.writeFileSync( file, data, options )
    blocksProcesed = panel.blocks.map(block => {
        return {
...
const cep = function (panel, context) {
    ...
    // Save Panel each time after 100 iterations
    if (processId % 100 === 0) {
        panelWork = panel.blocks.map(block => {
            return {
                x: block.pos.x.toNumber(),
                y: block.pos.y.toNumber(),
                width: block.size.x.toNumber(),
