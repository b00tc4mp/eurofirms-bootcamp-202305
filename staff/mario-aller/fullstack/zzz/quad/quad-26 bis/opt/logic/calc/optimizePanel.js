...
const optimizePanel = function (panel) {
    ...
    const cep = function (panel, context) {
        ...
        if (panel.blocskPlacedAll()) {
            ...
            function nextVertex(vertexIndex = 0) {
                if (vertexIndex === vertexs.length) return

                const vertex = vertexs[vertexIndex]
                // panel.blocks.forEach(async (block, indexBlock) => {
                function nextBlock(blockIndex = 0) {
                    if (blockIndex === panel.blocks.length) return

                    const block = panel.blocks[blockIndex]
                    if (!block.isPlaced()) {
                        const posIni = new Dimension2D(vertex.x.value, vertex.y.value)
                        function nextQuadrant(quadrant = 1) {
                            if (quadrant === 5) return
                            ...
                            function nextRotation(rotation = 0) {
                                ...
                                if (validBlock) {
                                    ...
                                    panel2.blocks[blockIndex].pos.x.value = posIni.x.value
                                    panel2.blocks[blockIndex].pos.y.value = posIni.y.value
                                    panel2.blocks[blockIndex].orientation = rotation
                                    panel2.status = 3

                                    // Save success panel each time after 100 iterations
                                    if (processId % 100 === 0) {
                                        if (!panelSaving) {
                                            console.log('- Go for saving -')
                                            panelSaving = true
                                            savePanel(panel)
                                                .then(() => {
                                                    panelSaving = false
