...
const sto = new Storage(name)

const operation = process.argv[3].toLowerCase()
switch (operation) {
    case 'add':
        if (!errorArg(6, 'add: error arg')) sto.itemPut(process.argv[4], process.argv[5])
        break
    case 'upt':
        if (!errorArg(7, 'update: error arg')) sto.itemUpdate(process.argv[4], process.argv[5], process.argv[6])
        break
    case 'del':
        if (!errorArg(5, 'delete: error arg')) sto.itemDelete(process.argv[4])
        break
