...
const { updatePanelStatusToOptimize } = require('.')
const { mongoose, models: { UserModel, PanelModel } } = require('dat')
const { expect } = require('chai')

debugger
describe('updatePanelStatusToOptimize', () => {
    ...
    it('succeeds on updatePanelStatusToOptimize', () =>
        updatePanelStatusToOptimize(userId, panelId)
            .then(() => PanelModel.findById(panelId)
                .then(panel => {
                    expect(panel.status).to.equal(1)
                })
            ))
    it('fails on user does not exist', () =>
        updatePanelStatusToOptimize(userId.slice(6) + 'ffffff', panelId)
            .catch(error => {
    ...
    it('fails on panel does not exist', () =>
        updatePanelStatusToOptimize(userId, panelId.slice(6) + 'ffffff')
            .catch(error => {
                expect(error).to.be.instanceOf(Error)
    ...
    it('fails on user cannot update', () => UserModel.create({ name, surname, zip, email: email2, password })
        ...
        .then(user => {
            userId2 = user.id
            return updatePanelStatusToOptimize(userId2, panelId)
                .catch(error => {
                    expect(error).to.be.instanceOf(Error)
